...
const MASCOTS = ["🦊", "🐰", "🐨", "🐼", "🐻", "🦁", "🐯", "🐶", "🐱", "🐥", "🦉", "🦄"];
const MASCOT_NAMES = ["Cáo Nhỏ", "Thỏ Bông", "Koala Hiền", "Gấu Trúc", "Gấu Nâu", "Sư Tử Con", "Hổ Nhỏ", "Cún Con", "Mèo Ấm", "Chíp Chíp", "Cú Trí Tuệ", "Kỳ Lân"];
...
const getAvatarByEmail = (email) => {
  ...
  const colors = ["#E07A5F", "#3D5A80", "#81B29A", "#F28482", "#F4A261", "#E76F51", "#2A9D8F", "#457B9D"];
  return {
...
const generateSimulatedAIAdvice = (question, parentAns, parentEmo, childAns, childEmo) => {
  const pEmoVi = { happy: "Vui vẻ", anxious: "Lo âu", hopeful: "Hy vọng", stressed: "Áp lực" }[parentEmo] || "Bình thường";
  const cEmoVi = { happy: "Vui vẻ", anxious: "Lo âu", hopeful: "Hy vọng", stressed: "Áp lực" }[childEmo] || "Bình thường";
  ...
  let focusPoints = [];
  if (lowercaseP.includes("học") || lowercaseC.includes("học") || lowercaseP.includes("điểm") || lowercaseC.includes("điểm")) {
    focusPoints.push("áp lực học tập và kỳ vọng điểm số");
  }
  if (lowercaseP.includes("điện thoại") || lowercaseC.includes("điện thoại") || lowercaseP.includes("game") || lowercaseC.includes("game")) {
    focusPoints.push("việc cân bằng thời gian sử dụng thiết bị công nghệ");
  }
  if (lowercaseP.includes("nghe") || lowercaseC.includes("nghe") || lowercaseP.includes("nói") || lowercaseC.includes("nói")) {
    focusPoints.push("cách thức lắng nghe chủ động và đối thoại ôn hòa");
  }

  const topicName = focusPoints.length > 0 ? focusPoints.join(" cũng như ") : "sự khác biệt trong thói quen và suy nghĩ hàng ngày";
...
function App() {
  ...
  // Sandbox AI thử nghiệm
  const [sandboxQuestion, setSandboxQuestion] = useState(PRESET_AI_QUESTIONS[0]);
  const [sandboxParentAns, setSandboxParentAns] = useState("Bố mẹ muốn con tập trung học hành chăm chỉ, thi đỗ trường chuyên lớp chọn để sau này đỡ khổ. Bố mẹ đi làm vất vả cũng chỉ vì tương lai của con thôi.");
  const [sandboxParentEmo, setSandboxParentEmo] = useState("anxious");
  const [sandboxChildAns, setSandboxChildAns] = useState("Con biết bố mẹ muốn tốt cho con, nhưng lịch học thêm dày đặc làm con mệt mỏi lắm. Nhiều lúc con chỉ muốn được nghỉ ngơi chơi thể thao một chút mà bố mẹ mắng lười học.");
  ...
  const handleJoinRoom = (e) => {
    ...
    const foundRoomIndex = rooms.findIndex(r => r.id.toUpperCase() === joinRoomId.toUpperCase());
    if (foundRoomIndex === -1) {
  ...
  const getMyUsernameInRoom = () => {
    ...
    return sessionStorage.getItem(`HN_room_username_${activeRoom.id}`) || 'Thành viên';
  };
  ...
  return (
    <>
      ...
      <header>
        <div className="container navbar">
          ...
          <div className="nav-actions">
            {/* Theme Toggle Button */}
            <button className="theme-toggle" onClick={toggleTheme} title="Thay đổi giao diện sáng/tối">
              {theme === 'light' ? '🌙' : '☀️'}
            </button>
            ...
            {currentUser ? (
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button className="btn btn-secondary btn-sm" onClick={() => navigateTo('login')}>Đăng Nhập</button>
                <button className="btn btn-primary btn-sm" onClick={() => navigateTo('register')}>Đăng Ký</button>
              </div>
            )}
          </div>
        </div>
      ...
      <main className="container animate-fade">
        ...
        {['home', 'mechanism', 'ai-info', 'about', 'contact'].includes(currentView) && (
          ...
          <div>
            <section className="hero-section">
              <div className="hero-content">
                <h1>Đôi khi yêu thương không có nghĩa là đã <span>hiểu nhau.</span></h1>
                <p>
            ...
            <section id="create-room-section" style={{ scrollMarginTop: '100px', paddingTop: '20px' }}>
              <div style={{ textAlign: 'center', marginBottom: '40px' }}>
                <h2 style={{ fontSize: '32px', marginBottom: '12px' }}>Không Gian Kết Nối Gia Đình</h2>
                <p style={{ color: 'var(--text-muted)', maxWidth: '600px', margin: '0 auto' }}>
                  Hãy tạo phòng kiểm tra mới cho gia đình bạn hoặc tham gia phòng đã có sẵn để cùng nhau chia sẻ những điều khó nói.
                </p>
              </div>

              {/* Display errors inside room creation/joining */}
              {roomError && (
                <div style={{ backgroundColor: '#FADBD8', color: '#78281F', padding: '16px', borderRadius: '12px', marginBottom: '30px', fontWeight: '600', border: '1.5px solid #F5B7B1', textAlign: 'center' }}>
                  ⚠️ {roomError}
                </div>
              )}
              ...
              <div style={{ maxWidth: '480px', margin: '0 auto' }} className="animate-slide">
                ...
                {activeRoomTab === 'join' && (
                  <div className="card animate-fade" id="join-room-section">
                    ...
                    <p className="card-desc" style={{ marginBottom: '20px' }}>
                      Nhập mã ID phòng và mật khẩu do người thân gửi để cùng tham gia kết nối tâm hồn.
                    </p>
        ...
        {currentView === 'ai-info' && (
          <div className="animate-slide">
            <div style={{ textAlign: 'center', marginBottom: '40px', maxWidth: '800px', margin: '0 auto 40px' }}>
              <h2 style={{ fontSize: '32px', marginBottom: '12px' }}>Công Nghệ AI Thấu Cảm Không Chỉ Trích</h2>
              <p style={{ color: 'var(--text-muted)' }}>
                Điểm cốt lõi của "Hiểu Nhau" là mô hình ngôn ngữ lớn được tinh chỉnh riêng về tâm lý gia đình. AI đóng vai trò như một chuyên gia hòa giải trung lập, thấu cảm và ấm áp.
              </p>
            </div>
            ...
            <div className="features-grid">
              ...
              <div className="card">
                ...
                <p className="card-desc">
                  Bằng cách tích hợp biểu tượng cảm xúc (Vui vẻ, lo lắng, hy vọng, áp lực) vào câu trả lời, AI phân tích sự bất đối xứng giữa câu chữ và tâm trạng để đưa ra lời khuyên phù hợp nhất.
                </p>
              </div>
              ...
              <div className="card">
                ...
                <p className="card-desc">
                  Thay vì những lý thuyết sáo rỗng, AI đúc kết thành các câu thoại mẫu cụ thể để cha mẹ và con cái áp dụng trực tiếp vào bữa ăn hay buổi trò chuyện hàng ngày.
                </p>
              </div>
            </div>
            ...
            <section className="card" style={{ border: '2px solid var(--primary)' }}>
              <div style={{ textAlign: 'center', marginBottom: '24px' }}>
                <span style={{ backgroundColor: 'rgba(224, 122, 95, 0.12)', color: 'var(--primary)', padding: '6px 14px', borderRadius: '30px', fontSize: '12px', fontWeight: '700', textTransform: 'uppercase' }}>Trải Nghiệm Thử</span>
                <h3 style={{ fontSize: '24px', marginTop: '10px' }}>Hộp Cát Trải Nghiệm AI</h3>
                <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>
                  Thử nhập đáp án giả lập của cha mẹ và con cái để xem AI thấu cảm phân tích và đưa ra lời khuyên dịu mát như thế nào!
                </p>
              </div>
              ...
              <div className="ai-sandbox-grid">
                ...
                <div>
                  <div className="form-group">
                    ...
                    <select value={sandboxQuestion} onChange={(e) => setSandboxQuestion(e.target.value)}>
                  ...
                  <div className="form-group">
                    ...
                    <div style={{ marginTop: '8px', display: 'flex', gap: '8px', alignItems: 'center' }}>
                      ...
                      {['happy', 'anxious', 'hopeful', 'stressed'].map(emo => (
                        ...
                        >
                          {getEmotionIcon(emo).emoji} {getEmotionIcon(emo).text}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <button className="btn btn-primary" onClick={triggerSandboxAI} style={{ width: '100%' }}>
        ...
        {currentView === 'about' && (
          <div className="container-narrow animate-slide">
            <div style={{ textAlign: 'center', marginBottom: '40px' }}>
              <h2 style={{ fontSize: '32px', marginBottom: '12px' }}>Về Dự Án "Hiểu Nhau"</h2>
              <p style={{ color: 'var(--text-muted)' }}>
            ...
            <div className="card" style={{ marginBottom: '30px' }}>
              <h3 style={{ fontSize: '22px', marginBottom: '12px' }}>Sứ Mệnh Của Chúng Tôi</h3>
              <p style={{ color: 'var(--text-main)', fontSize: '15px', marginBottom: '16px' }}>
                Xã hội hiện đại với sự bùng nổ của mạng xã hội vô tình kéo dãn khoảng cách giữa cha mẹ và con cái. Sự khác biệt về hệ giá trị sống và cách thức giao tiếp dẫn đến những hiểu lầm tích tụ thành rào cản im lặng.
              </p>
              <p style={{ color: 'var(--text-main)', fontSize: '15px' }}>
                Chúng tôi tin rằng, cốt lõi của sự gắn kết không phải là việc áp đặt một chuẩn mực hoàn hảo, mà là xây dựng một cầu nối giao tiếp an toàn, nơi cả phụ huynh và học sinh đều cảm thấy tiếng nói của mình được lắng nghe, được trân trọng mà không lo sợ bị phán xét.
              </p>
            </div>

            <div className="card">
              <h3 style={{ fontSize: '22px', marginBottom: '12px' }}>Đội Ngũ Sáng Tạo</h3>
              <p style={{ color: 'var(--text-muted)', fontSize: '14.5px', marginBottom: '16px' }}>
                Dự án "Hiểu Nhau" được lập trình và phát triển bởi các kỹ sư công nghệ nhiệt huyết phối hợp với các cố vấn tâm lý học hành vi gia đình Việt Nam. Chúng tôi hy vọng ứng dụng này sẽ là người bạn đồng hành ấm áp trong mỗi mái nhà.
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', textAlign: 'center', marginTop: '20px' }}>
                <div style={{ padding: '16px', background: 'var(--accent-warm)', borderRadius: '12px' }}>
                  <span style={{ fontSize: '28px' }}>🤖</span>
                  <h4 style={{ fontSize: '16px', marginTop: '6px' }}>Đội Ngũ AI</h4>
                  <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Tinh chỉnh mô hình tâm lý thấu cảm</span>
                </div>
                <div style={{ padding: '16px', background: 'var(--accent-warm)', borderRadius: '12px' }}>
                  <span style={{ fontSize: '28px' }}>❤️</span>
                  <h4 style={{ fontSize: '16px', marginTop: '6px' }}>Nhà Tâm Lý Học</h4>
                  <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Xây dựng kịch bản & quy trình kết nối</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {currentView === 'contact' && (
          <div className="container-narrow animate-slide">
            <div style={{ textAlign: 'center', marginBottom: '40px' }}>
              <h2 style={{ fontSize: '32px', marginBottom: '12px' }}>Gửi Lời Nhắn Yêu Thương</h2>
              <p style={{ color: 'var(--text-muted)' }}>
                Nếu bạn có bất kỳ thắc mắc, đóng góp ý kiến hay muốn chia sẻ câu chuyện gắn kết gia đình của mình, hãy viết thư cho chúng tôi nhé!
              </p>
            </div>

            {contactSuccess && (
              <div style={{ backgroundColor: '#D4EFDF', color: '#196F3D', padding: '18px', borderRadius: '16px', marginBottom: '24px', fontWeight: '600', border: '1.5px solid #A9DFBF', textAlign: 'center' }}>
                🌸 {contactSuccess}
              </div>
            )}
        ...
        {currentView === 'login' && (
          <div className="container-mobile animate-slide">
            <div style={{ textAlign: 'center', marginBottom: '30px' }}>
              <h2 style={{ fontSize: '28px', marginBottom: '8px' }}>Chào Mừng Trở Lại</h2>
              <p style={{ color: 'var(--text-muted)' }}>Đăng nhập để xem danh sách phòng kết nối đã lưu của bạn.</p>
            </div>
            ...
            <div className="card">
              ...
              <div style={{ textAlign: 'center', fontSize: '14px', color: 'var(--text-muted)' }}>
                Chưa có tài khoản?{' '}
                <a onClick={() => navigateTo('register')} style={{ color: 'var(--primary)', fontWeight: '600', cursor: 'pointer', textDecoration: 'underline' }}>
                  Đăng ký ngay
                </a>
              </div>
            </div>
        ...
        {currentView === 'register' && (
          <div className="container-mobile animate-slide">
            <div style={{ textAlign: 'center', marginBottom: '30px' }}>
              <h2 style={{ fontSize: '28px', marginBottom: '8px' }}>Tạo Tài Khoản Mới</h2>
              <p style={{ color: 'var(--text-muted)' }}>Đăng ký tài khoản để ghi nhớ lịch sử thấu cảm của gia đình.</p>
            </div>
            ...
            <div className="card">
              ...
              <div style={{ textAlign: 'center', fontSize: '14px', color: 'var(--text-muted)' }}>
                Đã có tài khoản rồi?{' '}
                <a onClick={() => navigateTo('login')} style={{ color: 'var(--primary)', fontWeight: '600', cursor: 'pointer', textDecoration: 'underline' }}>
                  Đăng nhập
                </a>
              </div>
            </div>
          ...
          <div className="animate-slide">
            ...
            <div className="room-header-status">
              <div className="room-meta-group">
                <button className="btn btn-secondary btn-sm" onClick={handleLeaveRoom} style={{ padding: '8px 12px' }}>
                  ⬅ Rời Phòng
                </button>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  ...
                  <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Mã tham gia: <strong>{activeRoom.id}</strong> (Mật khẩu: {activeRoom.password})</span>
                </div>
              </div>
            ...
            {activeRoom.status === 'waiting' && (
              <div className="q-creation-workspace">
                ...
                <div className="card">
                  ...
                  <h3 style={{ fontSize: '22px', margin: '8px 0 16px' }}>Biên Soạn Câu Hỏi Của Bạn</h3>
                  <p style={{ color: 'var(--text-muted)', fontSize: '13.5px', marginBottom: '20px' }}>
                    Hãy đặt những câu hỏi mà bạn luôn băn khoăn về đối phương. Các câu hỏi nháp này được **bảo mật tuyệt đối** cho đến khi bắt đầu bài kiểm tra chung.
                  </p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  <div className="card" style={{ flex: 1 }}>
                    ...
                    {(() => {
                      ...
                      if (myQuestions.length === 0) {
                        return (
                          <p style={{ fontStyle: 'italic', color: 'var(--text-muted)', fontSize: '13.5px' }}>
                            Chưa có câu hỏi nào được thêm. Hãy soạn hoặc click chọn gợi ý bên trái.
                          </p>
                        );
                      }
                  ...
                  <div className="peer-drafting-status-card">
                    ...
                    <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                      Mọi biên soạn đều được ẩn kín hoàn toàn đối với đối phương. Giao diện chỉ hiển thị trạng thái chuẩn bị để kích hoạt sự kết nối chung khi cả hai đã sẵn sàng.
                    </p>
                  </div>
                </div>
            ...
            {activeRoom.status === 'quiz' && (
              <div className="animate-slide">
                {(() => {
                  ...
                  if (currentQuestionIndex >= totalQuestions || myAnswersCount === totalQuestions) {
                    return (
                      <div className="card test-box-main" style={{ textAlign: 'center', padding: '50px 30px' }}>
                        <span style={{ fontSize: '48px', animation: 'pulse 1.5s infinite' }}>⏳</span>
                        <h3 style={{ fontSize: '24px', margin: '16px 0 8px' }}>Đang Đợi Người Thân Hoàn Thành...</h3>
                        <p style={{ color: 'var(--text-muted)', fontSize: '15px', maxWidth: '500px', margin: '0 auto' }}>
                          Bạn đã hoàn thành xuất sắc tất cả câu trả lời của mình! Hãy thưởng thức một tách trà ấm trong khi chờ đối phương chia sẻ xong suy nghĩ của họ nhé.
                        </p>

                        <div style={{ marginTop: '30px', display: 'flex', gap: '8px', justifyContent: 'center' }}>
                          <div style={{ width: '10px', height: '10px', backgroundColor: 'var(--primary)', borderRadius: '50%', animation: 'pulse 1.2s infinite' }}></div>
                          <div style={{ width: '10px', height: '10px', backgroundColor: 'var(--primary)', borderRadius: '50%', animation: 'pulse 1.2s infinite 0.2s' }}></div>
                          <div style={{ width: '10px', height: '10px', backgroundColor: 'var(--primary)', borderRadius: '50%', animation: 'pulse 1.2s infinite 0.4s' }}></div>
                        </div>

                        {/* Helper tip for local testing simulation */}
                        <div style={{ marginTop: '40px', padding: '16px', backgroundColor: 'var(--accent-warm)', borderRadius: '12px', border: '1.5px dashed var(--border)' }}>
                          <span style={{ fontWeight: '700', fontSize: '13px', color: 'var(--primary)', display: 'block', marginBottom: '4px' }}>💡 Hướng dẫn mô phỏng 2 thiết bị:</span>
                          <p style={{ fontSize: '12.5px', color: 'var(--text-muted)' }}>
                            Hãy mở một tab trình duyệt khác ở chế độ ẩn danh (hoặc trình duyệt khác), tham gia phòng <strong>{activeRoom.id}</strong> bằng vai trò ngược lại và làm bài test. Cả hai tab sẽ lập tức đồng bộ chuyển sang màn hình duyệt kết quả!
                          </p>
                        </div>
                      </div>
                    );
                  ...
                  const activeQ = activeRoom.compiledQuestions[currentQuestionIndex];
                  const qCreatorText = activeQ.creator === 'parent' ? 'Câu hỏi từ Cha mẹ' : 'Câu hỏi từ Con cái';
                  ...
                  return (
                    <div className="card test-box-main">
                      ...
                      <div className="test-progress-bar-wrapper">
                        <div className="test-progress-fill" style={{ width: `${((currentQuestionIndex) / totalQuestions) * 100}%` }}></div>
                      </div>
                        ...
                        <div className="emotion-select-row">
                          {[
                            ...
                            <button 
                              ...
                              type="button"
                            >
                              <span className="emotion-icon-sim">{item.emoji}</span>
                              {item.label}
                            </button>
                          ))}
                        </div>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '30px' }}>
                        ...
                        >
                          {currentQuestionIndex === totalQuestions - 1 ? 'Nộp bài kiểm tra' : 'Tiếp tục câu sau'}
                        </button>
                      </div>
                    </div>
                  );
                  ...
                  return (
                    <div>
                      <div className="review-meta-bar">
                        ...
                        <span>Câu hỏi {revIdx + 1} trên tổng số {totalQ}</span>
                      </div>
                      ...
                      <div className="review-q-card-title">
                        ...
                        <h2 style={{ fontSize: '22px', marginTop: '6px' }}>{activeQ.text}</h2>
                        <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Tạo bởi: {activeQ.creator === 'parent' ? 'Phụ huynh' : 'Học sinh'}</span>
                      </div>
                      ...
                      <div style={{ textAlign: 'center', marginTop: '30px' }}>
                        <button className="btn btn-primary" onClick={handleNextReviewQuestion} style={{ minWidth: '200px' }}>
                          {revIdx === totalQ - 1 ? 'Xem báo cáo tổng hợp kết quả' : 'Tiếp tục duyệt câu sau ➜'}
                        </button>
                      </div>
                    </div>
                  );
                ...
                <div className="final-advice-split">
                  ...
                  <div className="final-advice-card parent">
                    ...
                    <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '16px' }}>
                      Những hành vi nhỏ có thể làm dịu cảm giác lo lắng của con cái:
                    </p>
                    <ul className="advice-list-items">
                      <li><strong>Ghi nhận nỗ lực:</strong> Con cái rất thèm khát sự ghi nhận. Thay vì chỉ ra lỗi sai ngay, hãy khen ngợi hành động cố gắng của con trước.</li>
                      <li><strong>Giảm câu hỏi phán xét:</strong> Hạn chế các câu hỏi tu từ mang tính kiểm soát như <em>"Con đi học kiểu gì thế?"</em>, đổi thành câu hỏi mở: <em>"Bố mẹ có thể giúp gì được con không?"</em>.</li>
                      <li><strong>Kiểm soát nỗi lo lắng:</strong> Hãy học cách tin tưởng và cho con cơ hội tự chịu trách nhiệm với những quyết định nhỏ trong cuộc sống.</li>
                    </ul>
                  </div>

                  {/* Advice for Child group */}
                  <div className="final-advice-card child">
                    <h3 style={{ color: 'var(--primary)' }}>🐰 Nhóm Thay Đổi Cho Con Cái</h3>
                    <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '16px' }}>
                      Những hành động giúp bố mẹ cảm thấy được tôn trọng và an tâm:
                    </p>
                    <ul className="advice-list-items">
                      <li><strong>Chủ động thông báo:</strong> Chia sẻ trước những kế hoạch nhỏ như giờ đi chơi, kết quả học tập để cha mẹ bớt hoài nghi và lo sợ.</li>
                      <li><strong>Đón nhận sự quan tâm:</strong> Hãy hiểu rằng lời cằn nhằn đôi khi chỉ là thói quen diễn đạt vụng về của tình thương cha mẹ.</li>
                      <li><strong>Bộc bạch cảm xúc ôn hòa:</strong> Thay vì im lặng hay đóng sập cửa, hãy dùng cấu trúc: <em>"Con rất muốn làm việc này, bố mẹ hãy cho con thử nghiệm một lần nhé."</em></li>
                    </ul>
                  </div>
      ...
      <footer>
        <div className="container">
          <p style={{ fontFamily: 'var(--font-heading)', fontWeight: '600', color: 'var(--secondary)' }}>
            🏡 "Gia đình là nơi bão dừng sau cánh cửa. Lắng nghe để thấu cảm sâu sắc."
          </p>
          <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
            © 2026 Dự Án Hiểu Nhau - Trợ Lý AI Hàn Gắn Khoảng Cách Thế Hệ. All rights reserved.
          </p>
        </div>
      </footer>
